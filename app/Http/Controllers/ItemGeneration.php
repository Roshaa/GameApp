...
class ItemGeneration extends Controller
{
    ...
    private static function getrandomstatvalue($level)
        {
    ...
    public static function missionitemgeneration($mobtier)
    {
        ...
        $itemvalue=self::getrandomstatvalue($level)*0.8;
        ...
        */
        ...
        switch ($itemrarityroll) {
            case $itemrarityroll == 'common':
                ...
                if ($itemtype == 'Weapon') {
                    $Items->stat1 = self::getrandomstatvalue($level) * 2;
                } else if ($itemtype == 'ProfessionTool') {
                    $Items->$dbrandomstatstring1 = self::getrandomstatvalue($level) * 2;
                } else {
                    $Items->stat1 = self::getrandomstatvalue($level);
                }
                break;
            case $itemrarityroll == 'uncommon':

                if ($itemtype == 'Weapon') {
                    $Items->stat1 = self::getrandomstatvalue($level) * 2.3;
                } else if ($itemtype == 'ProfessionTool') {
                    $Items->$dbrandomstatstring1 = self::getrandomstatvalue($level) * 2.3;
                } else {
                    $Items->stat1 = self::getrandomstatvalue($level);
                    $Items->$dbrandomstatstring1 = self::getrandomstatvalue($level);
                }
                break;
            case $itemrarityroll == 'rare':

                if ($itemtype == 'Weapon') {
                    $Items->stat1 = self::getrandomstatvalue($level) * 2.6;
                } else if ($itemtype == 'ProfessionTool') {
                    $Items->$dbrandomstatstring1 = self::getrandomstatvalue($level) * 2.6;
                } else {
                    $Items->stat1 = self::getrandomstatvalue($level);
                    $Items->$dbrandomstatstring1 = self::getrandomstatvalue($level);
                    $Items->$dbrandomstatstring2 = self::getrandomstatvalue($level);
            ...
            case $itemrarityroll == 'epic':
                ...
                if ($itemtype == 'Weapon') {
                    $Items->stat1 = self::getrandomstatvalue($level) * 3;
                } else if ($itemtype == 'ProfessionTool') {
                    $Items->$dbrandomstatstring1 = self::getrandomstatvalue($level) * 3;
                } else {
                    $Items->stat1 = self::getrandomstatvalue($level);
                    $Items->$dbrandomstatstring1 = self::getrandomstatvalue($level);
                    $Items->$dbrandomstatstring2 = self::getrandomstatvalue($level);
                    $Items->$dbrandomstatstring3 = self::getrandomstatvalue($level);
                }
    ...
    public static function shopitemgeneration($itemrestock)
    {
        ...
        */
        ...
        $shop = user_shops::where('user_id', '=', $user_id)->first();
        $Items->save();
        $LastItem = Items::where('user_owner_id', '=', $user_id)->orderBy('id', 'desc')->first();
        $shop->$itemrestock = $LastItem->id;
        $shop->save();
